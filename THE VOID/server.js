...
const PROFILE_PIC_BODY_BYTES =
  Math.ceil(PROFILE_PIC_MAX_BYTES * 1.4) + 512 * 1024;
const SESSION_TTL_MS = 12 * 60 * 60 * 1000;
const ACTIVE_SESSION_IDLE_MS = positiveNumberEnv(
  "THE_VOID_ACTIVE_SESSION_IDLE_MS",
  30 * 60 * 1000,
  1000,
);
const ADMIN_LOGIN_MAX_FAILED_ATTEMPTS = positiveNumberEnv(
  "THE_VOID_ADMIN_LOGIN_MAX_FAILED_ATTEMPTS",
  2,
  1,
);
const ADMIN_LOGIN_LOCK_MS = positiveNumberEnv(
  "THE_VOID_ADMIN_LOGIN_LOCK_MS",
  3 * 60 * 60 * 1000,
  60 * 1000,
);
const USER_LOGIN_MAX_FAILED_ATTEMPTS = positiveNumberEnv(
  "THE_VOID_USER_LOGIN_MAX_FAILED_ATTEMPTS",
  4,
  1,
);
const USER_LOGIN_LOCK_MS = positiveNumberEnv(
  "THE_VOID_USER_LOGIN_LOCK_MS",
  30 * 60 * 1000,
  60 * 1000,
);
const WALLPAPER_UPLOAD_LIMIT_PER_WINDOW = positiveNumberEnv(
...
const ONE_DEVICE_LOGIN_MESSAGE =
  "This profile is already logged in on another device. Logout from the other device to login here.";
...
const SUPABASE_URL = String(
  process.env.SUPABASE_URL || process.env.THE_VOID_SUPABASE_URL || "",
)
  .trim()
  .replace(/\/+$/, "");
const SUPABASE_SERVICE_ROLE_KEY = String(
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
    process.env.THE_VOID_SUPABASE_SERVICE_ROLE_KEY ||
    "",
).trim();
const SUPABASE_BUCKET =
  String(
    process.env.SUPABASE_BUCKET ||
      process.env.THE_VOID_SUPABASE_BUCKET ||
      "wallpapers",
  ).trim() || "wallpapers";
const SUPABASE_PROFILE_BUCKET =
  String(
    process.env.SUPABASE_PROFILE_BUCKET ||
      process.env.THE_VOID_PROFILE_BUCKET ||
      "profile-pics",
  ).trim() || "profile-pics";
const FORCE_BUNDLE_SEED = /^true$/i.test(
  String(process.env.THE_VOID_FORCE_BUNDLE_SEED || "").trim(),
);
const AUTO_SYNC_STORAGE = !/^false$/i.test(
  String(process.env.THE_VOID_AUTO_SYNC_STORAGE || "true").trim(),
);
const ENFORCE_ONE_ACCOUNT_PER_IP = !/^false$/i.test(
  String(process.env.THE_VOID_ENFORCE_ONE_ACCOUNT_PER_IP || "true").trim(),
);
const IP_HASH_SECRET = String(
  process.env.THE_VOID_IP_HASH_SECRET ||
    SUPABASE_SERVICE_ROLE_KEY ||
    process.env.THE_VOID_ADMIN_PASSWORD ||
    "the-void-dev-secret",
).trim();
...
const WALLPAPER_THUMB_SIZE = { width: 360, height: 640 };
const WALLPAPER_PREVIEW_SIZE = { width: 720, height: 1280 };
...
const configuredHash =
  process.env.THE_VOID_ADMIN_PASSWORD_HASH || sha256(runtimePassword);
...
const APPROVED_WALLPAPER_CACHE_MS = positiveNumberEnv(
  "THE_VOID_WALLPAPER_CACHE_MS",
  10000,
  1000,
);
if (
  !process.env.THE_VOID_ADMIN_PASSWORD &&
  !process.env.THE_VOID_ADMIN_PASSWORD_HASH
) {
...
if (USE_SUPABASE) {
  if (!createClient) {
    setupError =
      "Supabase dependency missing. Run npm install so @supabase/supabase-js is installed.";
  } else {
  ...
  setupError =
    "Supabase is not configured. Add SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in Render environment variables.";
} else {
  prepareLocalStorage();
}
function isRender() {
...
async function readAppSettingValue(key) {
  ...
  if (USE_SUPABASE) {
    ...
    if (!setupError && supabase) {
      ...
      return result.data?.value && typeof result.data.value === "object"
        ? result.data.value
        : {};
    }
  }
  ...
  if (USE_SUPABASE) {
    ...
    if (!setupError && supabase) {
      ...
      const result = await supabase
        .from("app_settings")
        .upsert(
          { key: safeKey, value: safeValue, updated_at: now },
          { onConflict: "key" },
        );
      if (result.error) throw result.error;
      return;
...
async function writeUploadLimitExemptions(users) {
  ...
  await writeAppSettingValue(UPLOAD_LIMIT_EXEMPTIONS_SETTING_KEY, {
    users: cleaned,
...
async function userHasUnlimitedUploads(creatorId) {
  ...
  if (!id) return false;
  if (!USE_SUPABASE) {
    const localUser = normalizeUserRow(
      readLocalUsers().find((user) => user.creatorId === id) || null,
    );
...
function retryAfterSecondsUntil(lockedUntilMs) {
  return Math.max(
    1,
    Math.ceil((Number(lockedUntilMs || 0) - Date.now()) / 1000),
  );
}
...
async function readAdminLoginProtection() {
  ...
  for (const [deviceHash, entry] of Object.entries(source)) {
    ...
    const parsedLockedUntilMs = Number.isFinite(lockedUntilMs)
      ? lockedUntilMs
      : 0;
    devices[deviceHash] = {
      failedAttempts:
        parsedLockedUntilMs && parsedLockedUntilMs <= Date.now()
          ? 0
          : Math.max(0, Number(entry.failedAttempts || 0) || 0),
      lockedUntilMs:
        parsedLockedUntilMs && parsedLockedUntilMs > Date.now()
          ? parsedLockedUntilMs
          : 0,
...
async function recordAdminLoginFailure(deviceHash) {
  ...
  return {
    ...
    remainingAttempts: Math.max(
      0,
      ADMIN_LOGIN_MAX_FAILED_ATTEMPTS - failedAttempts,
    ),
  };
}
...
async function recordUserLoginFailure(deviceHash) {
  ...
  return {
    ...
    remainingAttempts: Math.max(
      0,
      USER_LOGIN_MAX_FAILED_ATTEMPTS - failedAttempts,
    ),
  };
}
...
function securityHeaders() {
  return {
    ...
    "Content-Security-Policy":
      "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com data:; img-src 'self' data: blob: https:; media-src 'self' data: blob: https:; connect-src 'self' https:; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'",
  };
}
...
function expectedOrigins(req) {
  ...
  const proto =
    String(Array.isArray(protoHeader) ? protoHeader[0] : protoHeader)
      .split(",")[0]
      .trim() || "http";
...
function friendlySupabaseError(error) {
  const message = String(
    error?.message || error?.details || error || "Supabase setup failed.",
  );
  if (
    /relation .*wallpapers.*does not exist|relation .*app_settings.*does not exist|relation .*users.*does not exist|schema cache/i.test(
      message,
    )
  ) {
  ...
  if (/duplicate key value|unique constraint/i.test(message)) {
    if (/users_creator_name_key_unique|creator_name|username/i.test(message))
      return "USERNAME_TAKEN";
    if (/users_signup_ip_hash_unique|signup_ip_hash/i.test(message))
      return "Creator profile already exist on this IP.";
    if (/users_browser_key_hash_unique|browser_key/i.test(message))
      return "Creator profile already exist on this IP.";
  }
...
async function bootstrapSupabase() {
  if (!supabase) return;
  const settingsCheck = await supabase
    .from("app_settings")
    .select("key")
    .limit(1);
  if (settingsCheck.error) throw settingsCheck.error;
  const wallpaperCheck = await supabase
    .from("wallpapers")
    .select("id")
    .limit(1);
  ...
  const usersCheck = await supabase
    .from("users")
    .select(
      "creator_id,creator_name,password_hash,password_salt,signup_ip_hash,profile_pic_url,profile_pic_path",
    )
    .limit(1);
...
function runSupabaseStartupMaintenance() {
  if (!supabase || supabaseStartupMaintenancePromise)
    return supabaseStartupMaintenancePromise;
  supabaseStartupMaintenancePromise = (async () => {
    await seedBundledWallpapersOnce();
    await syncExistingStorageObjectsToDb();
    await backfillSupabaseWallpaperVariants();
  })()
    .catch((error) => {
      console.warn(
        "Supabase startup maintenance skipped:",
        friendlySupabaseError(error) || error.message || error,
      );
    })
    .finally(() => {
      supabaseStartupMaintenancePromise = null;
    });
  return supabaseStartupMaintenancePromise;
}
...
async function seedBundledWallpapersOnce() {
  ...
  for (const item of bundledDb.wallpapers) {
    ...
    const ext =
      path.extname(filename).replace(/^\./, "").toLowerCase() || "jpg";
    const mime =
      item.mime ||
      mimeForExt(path.extname(filename)) ||
    ...
    if (!publicUrl)
      throw new Error(`Could not create public URL for ${storagePath}.`);
    rows.push({
      id,
      title:
        String(item.title || "Untitled wallpaper")
          .trim()
          .slice(0, 80) || "Untitled wallpaper",
      creator:
        String(item.creator || "The Void")
          .trim()
          .slice(0, 60) || "The Void",
  ...
  console.log(
    `Seeded ${rows.length} bundled wallpapers into Supabase Storage/DB.`,
  );
}
...
function titleFromStoragePath(storagePath) {
  const name = path.basename(
    String(storagePath || ""),
    path.extname(String(storagePath || "")),
  );
  const cleaned = name
    .replace(
...
async function syncExistingStorageObjectsToDb() {
  ...
  for (const file of files) {
    if (
      !file.storagePath ||
      isWallpaperVariantStoragePath(file.storagePath) ||
      existingPaths.has(file.storagePath)
    )
      continue;
    const mime = String(
      file?.metadata?.mimetype ||
        ...
        mimeForExt(path.extname(file.storagePath)) ||
        "image/jpeg",
    );
    const publicUrlResult = supabase.storage
      .from(SUPABASE_BUCKET)
  ...
  console.log(
    `Recovered ${rows.length} existing Supabase Storage image(s) into the wallpapers table.`,
  );
}
...
async function uploadSingleSupabaseWallpaperVariant(id, buffer, variantName) {
  const size =
    variantName === "preview" ? WALLPAPER_PREVIEW_SIZE : WALLPAPER_THUMB_SIZE;
  const quality = variantName === "preview" ? 78 : 72;
  const out = await makeWallpaperVariantBuffer(buffer, size, quality);
  if (!out) return false;
  const upload = await supabase.storage
    .from(SUPABASE_BUCKET)
    .upload(wallpaperVariantStoragePath(id, variantName), out, {
...
async function backfillSupabaseWallpaperVariants() {
  ...
  for (let offset = 0; offset < 10000; offset += 1000) {
    ...
    if (result.error) {
      console.warn(
        "Supabase wallpaper variant backfill skipped:",
        friendlySupabaseError(result.error) ||
    ...
    for (const row of rows) {
      if (
        !row?.id ||
        !row?.storage_path ||
        isWallpaperVariantStoragePath(row.storage_path)
      )
        continue;
      checked += 1;
      try {
        ...
        for (const variantName of ["thumb", "preview"]) {
          const exists = await supabaseStorageObjectExists(
            wallpaperVariantStoragePath(row.id, variantName),
          );
        ...
        for (const variantName of missing) {
          if (
            await uploadSingleSupabaseWallpaperVariant(
              row.id,
              buffer,
              variantName,
            )
        ...
        console.warn(
          "Could not backfill wallpaper variants for",
          row.id,
          friendlySupabaseError(error) || error.message || error,
        );
      }
  ...
  if (generated) {
    console.log(
      `Backfilled ${generated} missing Supabase wallpaper variant(s) for ${checked} wallpaper(s).`,
    );
  }
}
...
function prepareLocalStorage() {
  ...
  ensureLocalWallpaperVariants().catch((error) =>
    console.warn(
      "Wallpaper variant generation skipped:",
      error.message || error,
    ),
  );
}
...
function parseImageDataUrl(dataUrl, maxBytes, label = "image") {
  ...
  if (!buffer.length || buffer.length > maxBytes) {
    const error = new Error(
      `${label.charAt(0).toUpperCase()}${label.slice(1)} must be under ${Math.floor(maxBytes / (1024 * 1024))} MB.`,
    );
    error.status = 400;
    throw error;
  }
...
async function cleanImageBuffer(buffer, options = {}) {
  if (!sharp) {
    const error = new Error(
      "Image validation dependency is missing. Run npm install before deploying.",
    );
    error.status = 500;
    throw error;
  }
  ...
  try {
    ...
    const error = new Error(
      `The uploaded ${label} is not a valid clean image file.`,
    );
    error.status = 400;
    throw error;
  }
  ...
  if (!metadata.width || !metadata.height) {
    const error = new Error(
      `The uploaded ${label} has invalid image dimensions.`,
    );
    error.status = 400;
    throw error;
  }
  if (
    ...
    const error = new Error(
      `The uploaded ${label} is too large in resolution.`,
    );
    error.status = 400;
    throw error;
  }
  ...
  let cleanBuffer;
  try {
    cleanBuffer = await image
      .webp({ quality: options.quality || 90, effort: 4 })
      .toBuffer();
  } catch {
    const error = new Error(
      `The uploaded ${label} could not be safely cleaned.`,
    );
    error.status = 400;
    throw error;
  }
  if (!cleanBuffer.length || cleanBuffer.length > maxBytes) {
    const error = new Error(
      `Cleaned ${label} must be under ${Math.floor(maxBytes / (1024 * 1024))} MB.`,
    );
    error.status = 400;
    throw error;
  }
...
function publicWallpaperDto(item) {
  return {
    ...
    thumbUrl: thumbUrlForItem(item),
    previewUrl: previewUrlForItem(item),
  };
}
...
function getRequestIp(req) {
  const header =
    ...
    "";
  const first = Array.isArray(header)
    ? header[0]
...
function activeSessionLastSeenAtMs(active) {
  const lastSeenAtMs = Date.parse(
    active?.updatedAt || active?.lastSeenAt || "",
  );
  return Number.isFinite(lastSeenAtMs) ? lastSeenAtMs : 0;
}
function activeSessionIsFresh(active) {
  return Boolean(
    active?.tokenHash && activeSessionExpiresAtMs(active) > Date.now(),
  );
}
  ...
  for (const [creatorId, active] of Object.entries(sessions || {})) {
    ...
    normalized[creatorId] = {
      ...
      browserKeyHash: String(
        active.browserKeyHash || active.deviceKeyHash || "",
      ),
      expiresAt: String(active.expiresAt || ""),
      updatedAt: String(
        active.updatedAt || active.lastSeenAt || active.createdAt || "",
      ),
    };
  }
  return normalized;
...
async function readActiveUserSessions() {
  ...
  for (const user of readLocalUsers()) {
    ...
    if (user.creatorId && activeSessionIsFresh(active))
      sessionsByCreator[user.creatorId] = active;
  }
...
async function setActiveUserSession(
  creatorId,
  tokenHash,
  expiresAt,
  browserKeyHash = "",
...
async function clearActiveUserSession({ creatorId = "", tokenHash = "" } = {}) {
  ...
  for (const [id, active] of Object.entries(sessionsByCreator)) {
    if (
      ...
      (tokenHash && active?.tokenHash === tokenHash)
    ) {
...
async function rejectLoginIfAnotherDevice(req, res, user, browserKeyHash = "") {
  ...
  if (active && active.tokenHash !== existingHash) {
    const sameKnownBrowser = Boolean(
      browserKeyHash &&
    ...
    if (!sameKnownBrowser && activeLockStillInUse) {
      sendJson(res, 409, {
        error: ONE_DEVICE_LOGIN_MESSAGE,
...
function adminUserDto(user, counts = {}, unlimitedUploads = false) {
  ...
  return {
    ...
    unlimitedUploads: Boolean(unlimitedUploads),
    uploadLimitText: unlimitedUploads
      ? "Unlimited uploads"
      : `${WALLPAPER_UPLOAD_LIMIT_PER_WINDOW} uploads / 24h`,
  };
}
async function createUserSession(
  ...
  sendJson(
    res,
    status,
    { ok: true, user: publicUserDto(user), ...extraPayload },
...
function normalizeUserRow(row) {
  ...
  return {
    ...
    unlimitedUploads: Boolean(row.unlimited_uploads || row.unlimitedUploads),
  };
}
...
async function supabaseSignupLockExists(signupIpHash, browserKeyHash) {
  ...
  for (const check of checks) {
    const result = await supabase
      .from("users")
      .select(
        "creator_id,creator_name,auth_type,password_hash,password_salt,signup_ip_hash,browser_key_hash",
      )
  ...
  try {
    ...
    if (USE_SUPABASE) {
      ...
      return sendJson(res, 200, {
        exists: Boolean((result.data || []).length),
      });
    }
    const exists = readLocalUsers().some(
      (user) => normalizeUsername(user.creatorName) === username,
    );
    return sendJson(res, 200, { exists });
  } catch (error) {
    sendJson(res, 400, {
      error: friendlySupabaseError(error) || "Could not check username.",
    });
  }
}
...
async function findUserByUsername(username) {
  ...
  if (USE_SUPABASE) {
    const result = await supabase
      .from("users")
      .select(
        "creator_id,creator_name,auth_type,email,password_hash,password_salt,signup_ip_hash,browser_key_hash,profile_pic_path,profile_pic_url,created_at,updated_at,last_login_at",
      )
  ...
  const found = readLocalUsers().find(
    (user) => normalizeUsername(user.creatorName) === normalized,
  );
...
async function signupUser(req, res) {
  ...
  try {
    ...
    const browserKey = String(payload.browserKey || "")
      .trim()
      .slice(0, 128);
    ...
    const browserKeyHash = browserKey
      ? hashScoped(browserKey, "browser-key")
      : null;
    ...
    if (existingUser) {
      if (
        ...
        return await createUserSession(
          ...
          {
            alreadyExisted: true,
            message: "Profile already exist, Loging in.",
          },
          browserKeyHash,
        );
      }
    ...
    if (USE_SUPABASE) {
      if (
        ENFORCE_ONE_ACCOUNT_PER_IP &&
        (await supabaseSignupLockExists(signupIpHash, browserKeyHash))
      ) {
        return sendJson(res, 409, {
          error: "Creator profile already exist on this IP.",
        });
      }
      const insertPayload = {
      ...
      let insert = await supabase
        ...
        .select(
          "creator_id,creator_name,auth_type,email,profile_pic_path,profile_pic_url,created_at,last_login_at",
        )
        .maybeSingle();
      if (insert.error) {
        const msg = String(insert.error.message || "");
        if (/creator_name|username/i.test(msg))
          return sendJson(res, 409, { error: "USERNAME_TAKEN" });
    ...
    const createdUser = await mutateLocalUsers((users) => {
      if (
        users.some((user) => normalizeUsername(user.creatorName) === username)
      ) {
      ...
      if (ENFORCE_ONE_ACCOUNT_PER_IP) {
        ...
        for (const user of users) {
          const sameIp =
            user.signupIpHash && user.signupIpHash === signupIpHash;
          const sameBrowser =
            browserKeyHash &&
            user.browserKeyHash &&
            user.browserKeyHash === browserKeyHash;
        ...
        if (hasActiveSignupLock) {
          const error = new Error("Creator profile already exist on this IP.");
          error.status = 409;
          throw error;
        }
      }
    ...
    const message =
      error.message === "USERNAME_TAKEN"
        ? "USERNAME_TAKEN"
        : friendlySupabaseError(error) || error.message || "Signup failed.";
  ...
  try {
    ...
    if (locked) {
      ...
      return sendJson(
        ...
        423,
        lockoutPayload(
          `Too many wrong login attempts from this device. Try again in ${formatMinutes(retryAfterSeconds)}.`,
    ...
    if (
      ...
      if (failure.locked) {
        ...
        return sendJson(
          ...
          423,
          lockoutPayload(
            `Too many wrong login attempts from this device. Login is locked for ${formatMinutes(retryAfterSeconds)}.`,
            failure.lockedUntilMs,
          ),
        );
      }
      return sendJson(res, 401, {
        error: `Invalid username or password, remaining ${failure.remainingAttempts} attempt${failure.remainingAttempts === 1 ? "" : "s"} left.`,
    ...
    sendJson(res, error.status || 400, {
      error: friendlySupabaseError(error) || error.message || "Login failed.",
    });
  }
}
async function logoutUser(req, res) {
  ...
  if (token) {
    ...
    try {
      ...
      console.error(
        "Could not clear active user session:",
        error.message || error,
      );
    }
  }
  ...
  try {
    if (USE_SUPABASE) {
      ...
      return sendJson(res, 200, {
        ...
        uploads,
        counts: uploadCounts(uploads),
      });
    }
    const uploads = readLocalDb()
      .wallpapers.filter((item) => item.creatorId === user.creatorId)
      .sort((a, b) =>
...
async function deleteProfile(req, res) {
  ...
  try {
    ...
    if (!password)
      return sendJson(res, 400, {
        error: "Enter your password to delete profile.",
      });
    const fullUser = await findUserById(user.creatorId);
    if (
      !fullUser ||
      !verifyPassword(password, fullUser.passwordSalt, fullUser.passwordHash)
    ) {
    ...
    if (USE_SUPABASE) {
      ...
      const wallpaperPaths = wallpapers
      ...
      const storageErrors = [];
      storageErrors.push(
        ...(await removeStorageObjects(SUPABASE_BUCKET, wallpaperPaths)),
      );
      if (fullUser.profilePicPath) {
        storageErrors.push(
          ...(await removeStorageObjects(SUPABASE_PROFILE_BUCKET, [
            fullUser.profilePicPath,
          ])),
        );
      }
      ...
      if (storageErrors.length) {
        payload.warning =
          "Profile deleted, but some Storage files may need manual cleanup.";
      }
    ...
    const removedWallpaperFiles = await mutateLocalDb((db) => {
      const removed = db.wallpapers.filter(
        (item) => item.creatorId === user.creatorId,
      );
      db.wallpapers = db.wallpapers.filter(
        (item) => item.creatorId !== user.creatorId,
      );
    ...
    const removedUser = await mutateLocalUsers((users) => {
      const index = users.findIndex(
        (entry) => entry.creatorId === user.creatorId,
      );
      if (index < 0) {
    ...
    if (removedUser?.profilePicPath) {
      const avatarPath = path.join(
        LOCAL_PROFILE_PIC_DIR,
        path.basename(removedUser.profilePicPath),
      );
...
async function parseProfilePicPayload(raw) {
  const payload = JSON.parse(raw || "{}");
  const { buffer } = parseImageDataUrl(
    payload.dataUrl,
...
async function updateProfilePicture(req, res) {
  ...
  try {
    ...
    if (USE_SUPABASE) {
      ...
      const publicUrl = publicUrlResult?.data?.publicUrl;
      if (!publicUrl)
        throw new Error("Could not create public URL for profile picture.");
      const update = await supabase
        .from("users")
      ...
      if (
        ...
        currentUser.profilePicPath !== storagePath
      ) {
        try {
          await supabase.storage
            .from(SUPABASE_PROFILE_BUCKET)
            .remove([currentUser.profilePicPath]);
        } catch {}
      }
      return sendJson(res, 200, {
        ok: true,
        message: "Profile picture updated.",
        user: publicUserDto(normalizeUserRow(update.data)),
      });
    }
    ...
    return sendJson(res, 200, {
      ok: true,
      message: "Profile picture updated.",
      user: publicUserDto(normalizeUserRow(updatedUser)),
    });
  } catch (error) {
    if (storagePath) {
      try {
    ...
    if (USE_SUPABASE) {
      ...
      return sendJson(res, 200, {
        users: (result.data || []).map((row) =>
          publicUserDto(normalizeUserRow(row)),
        ),
      });
    }
    const users = readLocalUsers()
      ...
      .sort((a, b) =>
        String(a.creatorName).localeCompare(String(b.creatorName)),
      )
      .slice(0, 8)
      .map(publicUserDto);
  ...
  try {
    ...
    if (USE_SUPABASE) {
      const result = await supabase
        .from("wallpapers")
        .select(
          "id,title,creator,creator_id,status,public_url,storage_path,mime,created_at,approved_at",
        )
        .eq("creator_id", user.creatorId)
        .order("created_at", { ascending: false });
      if (result.error) throw result.error;
...
async function findUserById(creatorId) {
  ...
  return normalizeUserRow(
    readLocalUsers().find((user) => user.creatorId === creatorId) || null,
  );
}
async function countRecentWallpaperUploads(creatorId) {
  const cutoffIso = new Date(
    Date.now() - WALLPAPER_UPLOAD_WINDOW_MS,
  ).toISOString();
  if (USE_SUPABASE) {
    const result = await supabase
      .from("wallpapers")
  ...
  return readLocalDb().wallpapers.filter((item) => {
    ...
    return (
      ...
      createdAt >= Date.now() - WALLPAPER_UPLOAD_WINDOW_MS
    );
  }).length;
}
...
async function requireCreator(req, res) {
  ...
  if (active && active.tokenHash !== session.tokenHash) {
    ...
    sendJson(
      res,
      401,
      { error: ONE_DEVICE_LOGIN_MESSAGE, code: "ACTIVE_SESSION_REPLACED" },
      { "Set-Cookie": clearUserSessionCookie() },
    );
    return null;
  }
  try {
    await setActiveUserSession(
      ...
      session.browserKeyHash || "",
    );
  } catch (error) {
    console.error(
      "Could not refresh active user session:",
      error.message || error,
    );
  }
...
async function parseImagePayload(raw) {
  const payload = JSON.parse(raw || "{}");
  const title =
    String(payload.title || "Untitled wallpaper")
      .trim()
      .slice(0, 80) || "Untitled wallpaper";
  const creator = normalizeCreatorName(payload.creator) || "The Void";
  const creatorId = String(payload.creatorId || "").trim();
  const authType = ["password", "guest", "google"].includes(
    String(payload.authType || "").trim(),
  )
    ? String(payload.authType).trim()
    : "password";
  const { buffer } = parseImageDataUrl(
...
async function handleSupabaseUpload(req, res, user) {
  ...
  try {
    ...
    const { title, creator, creatorId, authType, mime, ext, buffer } =
    ...
    if (upload.error) throw upload.error;
    await uploadSupabaseWallpaperVariants(id, buffer, false);
    const publicUrlResult = supabase.storage
      .from(SUPABASE_BUCKET)
      .getPublicUrl(storagePath);
    const publicUrl = publicUrlResult?.data?.publicUrl;
    if (!publicUrl)
      throw new Error("Could not create public URL for uploaded wallpaper.");
    const now = new Date().toISOString();
    ...
    sendJson(res, 201, {
      ok: true,
      message: "Wallpaper submitted for approval.",
    });
  } catch (error) {
    if (storagePath) {
      try {
    ...
    sendJson(res, error.status || 400, {
      error: friendlySupabaseError(error) || "Upload failed.",
    });
  }
}
async function handleLocalUpload(req, res, user) {
  ...
  try {
    ...
    writeFileAtomic(savedFilePath, buffer);
    await generateLocalWallpaperVariants(id, buffer);
...
async function listSupabaseApproved(res) {
  try {
    const now = Date.now();
    if (
      approvedWallpapersCache &&
      now - approvedWallpapersCacheAt < APPROVED_WALLPAPER_CACHE_MS
    ) {
      return sendJson(
        res,
        200,
        { wallpapers: approvedWallpapersCache },
    ...
    const result = await supabase
      .from("wallpapers")
      .select(
        "id,title,creator,creator_id,auth_type,storage_path,public_url,mime,status,created_at,approved_at,updated_at",
      )
      .eq("status", "approved")
      .order("created_at", { ascending: false });
    if (result.error) throw result.error;
    ...
    if (approvedWallpapersCache) {
      return sendJson(
        res,
        200,
        { wallpapers: approvedWallpapersCache, stale: true },
        { "Cache-Control": "public, max-age=5" },
      );
    }
    sendJson(res, 500, {
      error: friendlySupabaseError(error) || "Could not load wallpapers.",
    });
  }
}
...
async function serveSupabaseMedia(req, res, id, asDownload = false) {
  try {
    const result = await supabase
      .from("wallpapers")
      .select(
        "id,title,creator,creator_id,auth_type,storage_path,public_url,mime,status",
      )
      .eq("id", id)
      .maybeSingle();
    if (result.error) throw result.error;
    ...
    const storagePath =
      variant === "thumb"
        ? wallpaperVariantStoragePath(item.id, "thumb")
        : variant === "preview"
          ? wallpaperVariantStoragePath(item.id, "preview")
          : item.storagePath;
    let file = await supabase.storage
      .from(SUPABASE_BUCKET)
      .download(storagePath);
    let responseMime = variant === "original" ? item.mime : "image/webp";
    if (file.error && variant !== "original") {
      file = await supabase.storage
        .from(SUPABASE_BUCKET)
    ...
    res.writeHead(200, {
      "Content-Type": responseMime || "application/octet-stream",
      "Content-Length": buffer.length,
      "Cache-Control":
        item.status === "approved" ? "public, max-age=86400" : "no-store",
      "Content-Disposition": `${asDownload ? "attachment" : "inline"}; filename="${safeDownloadName(item.title)}.${extForMime(responseMime || item.mime) || "jpg"}"`,
    });
    res.end(buffer);
  } catch (error) {
    res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" });
...
async function ensureLocalWallpaperVariants() {
  if (!sharp) return;
  fs.mkdirSync(path.join(LOCAL_VARIANT_DIR, "thumbs"), { recursive: true });
  fs.mkdirSync(path.join(LOCAL_VARIANT_DIR, "previews"), { recursive: true });
  const db = readLocalDb();
  for (const item of db.wallpapers || []) {
    ...
    try {
      await generateLocalWallpaperVariants(
        item.id,
      ...
      console.warn(
        "Could not generate variants for",
        item.id,
        error.message || error,
      );
    }
...
function wallpaperVariantUrlForItem(item, variant) {
  ...
  if (USE_SUPABASE) {
    const publicUrl = supabase?.storage
      ?.from(SUPABASE_BUCKET)
      ?.getPublicUrl(wallpaperVariantStoragePath(item.id, variant))
      ?.data?.publicUrl;
    return publicUrl || mediaUrlForItem(item);
  }
  return (
    "/media/" +
    item.id +
    "?variant=" +
    (variant === "preview" ? "preview" : "thumb")
  );
}
...
async function generateLocalWallpaperVariants(id, buffer, overwrite = false) {
  ...
  if (overwrite || !fs.existsSync(thumbPath)) {
    const thumb = await makeWallpaperVariantBuffer(
      buffer,
      WALLPAPER_THUMB_SIZE,
      72,
    );
    if (thumb) writeFileAtomic(thumbPath, thumb);
  }
  if (overwrite || !fs.existsSync(previewPath)) {
    const preview = await makeWallpaperVariantBuffer(
      buffer,
      WALLPAPER_PREVIEW_SIZE,
      78,
    );
...
async function uploadSupabaseWallpaperVariants(id, buffer, upsert = false) {
  ...
  const variants = [
    { name: "thumb", size: WALLPAPER_THUMB_SIZE, quality: 72 },
    { name: "preview", size: WALLPAPER_PREVIEW_SIZE, quality: 78 },
  ];
  for (const variant of variants) {
    const out = await makeWallpaperVariantBuffer(
      buffer,
      variant.size,
      variant.quality,
    );
    ...
    const upload = await supabase.storage
      .from(SUPABASE_BUCKET)
      .upload(wallpaperVariantStoragePath(id, variant.name), out, {
        contentType: "image/webp",
        cacheControl: "31536000",
        upsert,
      });
...
function serveLocalMedia(req, res, id, asDownload = false) {
  ...
  const variantPath =
    variant === "thumb"
      ? getLocalVariantFilePath(item.id, "thumb")
      : variant === "preview"
        ? getLocalVariantFilePath(item.id, "preview")
        : "";
  const filePath =
    variantPath && fs.existsSync(variantPath) ? variantPath : originalPath;
  ...
  const responseMime =
    variantPath && filePath === variantPath
      ? "image/webp"
      : item.mime || mimeForExt(path.extname(filePath));
  res.writeHead(200, {
    "Content-Type": responseMime,
    "Cache-Control":
      item.status === "approved" && !asDownload
        ? "public, max-age=86400"
        : "no-store",
    "Content-Disposition": `${asDownload ? "attachment" : "inline"}; filename="${safeDownloadName(item.title)}.${extForMime(responseMime) || path.extname(filePath).replace(/^./, "") || "jpg"}"`,
  });
  fs.createReadStream(filePath).pipe(res);
}
...
async function login(req, res) {
  try {
    ...
    const { password, browserKey } = JSON.parse(raw || "{}");
    ...
    if (locked) {
      ...
      return sendJson(
        ...
        423,
        lockoutPayload(
          `Too many wrong admin login attempts from this device. Try again in ${formatHours(retryAfterSeconds)}.`,
          locked.lockedUntilMs,
        ),
      );
    }
    ...
    if (!ok) {
      ...
      if (failure.locked) {
        ...
        return sendJson(
          ...
          423,
          lockoutPayload(
            `Wrong admin password entered too many times. Admin login is locked on this device for ${formatHours(retryAfterSeconds)}.`,
            failure.lockedUntilMs,
          ),
        );
      }
      return sendJson(res, 401, {
        error: `Invalid password. ${failure.remainingAttempts} attempt left before admin login locks on this device for 3 hours.`,
        remainingAttempts: failure.remainingAttempts,
      });
    }
...
async function listSupabasePending(res) {
  try {
    ...
    sendJson(res, 500, {
      error:
        friendlySupabaseError(error) || "Could not load pending wallpapers.",
    });
  }
}
function listLocalPending(res) {
  ...
  const pending = db.wallpapers
    ...
    .map((item) => ({
      ...
      thumbUrl: `/media/${item.id}?variant=thumb`,
      previewUrl: `/media/${item.id}?variant=preview`,
    }));
  sendJson(res, 200, { pending });
}
  ...
  try {
    ...
    sendJson(res, 200, {
      users: users.map((user) =>
        adminUserDto(
          user,
          counts.get(user.creatorId),
          uploadLimitExemptions[user.creatorId],
        ),
      ),
    });
  } catch (error) {
...
async function listLocalAdminUsers(req, res) {
  try {
    ...
    const filteredUsers = localUsers
      .filter(
        (user) =>
          !query || normalizeUsername(user.creatorName).startsWith(query),
      )
      .sort((a, b) => {
        if (query)
          return String(a.creatorName || "").localeCompare(
            String(b.creatorName || ""),
          );
    ...
    const users = filteredUsers.map((user) =>
      adminUserDto(
        user,
        counts.get(user.creatorId),
        user.unlimitedUploads || uploadLimitExemptions[user.creatorId],
      ),
    );
...
async function updateAdminUserUploadLimit(req, res, creatorId) {
  ...
  try {
    ...
    if (!USE_SUPABASE) {
      await mutateLocalUsers((users) => {
        const item = users.find((entry) => entry.creatorId === id);
        if (item) {
          item.unlimitedUploads = unlimited;
          item.updatedAt = new Date().toISOString();
        }
      });
    }
    ...
    sendJson(res, 200, {
      ok: true,
      creatorId: id,
      unlimitedUploads: unlimited,
      uploadLimitText: unlimited
        ? "Unlimited uploads"
        : `${WALLPAPER_UPLOAD_LIMIT_PER_WINDOW} uploads / 24h`,
      message: unlimited
        ? `@${user.creatorName} can now upload without the 24h limit.`
        : `@${user.creatorName} is back to the normal upload limit.`,
    });
  } catch (error) {
    sendJson(res, error.status || 400, {
      error:
...
async function deleteSupabaseAdminUser(res, creatorId) {
  try {
    ...
    const payload = {
      ...
      deletedUser: adminUserDto(
        user,
        countWallpapersByCreator(wallpapers).get(creatorId),
      ),
      deletedWallpapers: wallpapers.length,
    ...
    if (storageErrors.length) {
      payload.warning =
        "User was deleted, but some Storage files may need manual cleanup.";
...
async function deleteLocalAdminUser(res, creatorId) {
  try {
    ...
    sendJson(res, 200, {
      ...
      deletedUser: adminUserDto(removedUser, {
        total: removedWallpaperFiles.length,
      }),
      deletedWallpapers: removedWallpaperFiles.length,
      clearedSignupLock: Boolean(
        removedUser.signupIpHash || removedUser.browserKeyHash,
      ),
    });
  } catch (error) {
    sendJson(res, error.status || 400, {
...
async function cleanupSupabaseAdminUserLocks(res) {
  try {
    ...
    const staleRows = rows.filter(
      (row) =>
        !isActiveSignupLockUser(row) &&
        (row.signup_ip_hash || row.browser_key_hash),
    );
    let clearedRows = 0;
    ...
    sendJson(res, 200, {
      ok: true,
      message:
        "Cleaned stale signup/session locks. Active users keep their IP/device locks.",
  ...
  try {
    ...
    await mutateLocalUsers((users) => {
      for (const user of users) {
        ...
        if (
          !isActiveSignupLockUser(user) &&
          (user.signupIpHash || user.browserKeyHash)
        ) {
          user.signupIpHash = "";
          user.browserKeyHash = "";
          clearedRows += 1;
    ...
    sendJson(res, error.status || 400, {
      error: error.message || "Could not clean stale locks.",
    });
  }
}
...
async function updateOwnSupabaseWallpaperDetails(req, res, id, user) {
  try {
    ...
    if (result.error) throw result.error;
    if (!result.data)
      return sendJson(res, 404, { error: "Wallpaper not found." });
    clearApprovedWallpapersCache();
    sendJson(res, 200, {
      ok: true,
    ...
    sendJson(res, error.status || 400, {
      error: friendlySupabaseError(error) || "Could not update wallpaper.",
    });
  }
}
async function updateOwnLocalWallpaperDetails(req, res, id, user) {
  try {
    ...
    const updated = await mutateLocalDb((db) => {
      const item = db.wallpapers.find(
        (entry) => entry.id === id && entry.creatorId === user.creatorId,
      );
      if (!item) {
    ...
    sendJson(res, error.status || 400, {
      error: error.message || "Could not update wallpaper.",
    });
  }
}
async function updateSupabaseWallpaperDetails(req, res, id) {
  try {
    ...
    sendJson(res, error.status || 400, {
      error:
        friendlySupabaseError(error) || "Could not update wallpaper details.",
    });
  }
}
async function updateLocalWallpaperDetails(req, res, id) {
  try {
    ...
    sendJson(res, error.status || 400, {
      error: error.message || "Could not update wallpaper details.",
    });
  }
}
  ...
  try {
    ...
    if (status === "rejected" && result.data.storage_path) {
      try {
        await supabase.storage
          .from(SUPABASE_BUCKET)
          .remove([result.data.storage_path]);
      } catch {}
    }
    clearApprovedWallpapersCache();
    ...
    sendJson(res, 400, {
      error: friendlySupabaseError(error) || "Moderation failed.",
    });
  }
}
...
async function deleteOwnSupabaseWallpaper(res, id, user) {
  try {
    ...
    const storageErrors = await removeStorageObjects(SUPABASE_BUCKET, [
      find.data.storage_path,
    ]);
    const payload = { ok: true, message: "Wallpaper deleted." };
    if (storageErrors.length)
      payload.warning =
        "Wallpaper deleted, but the Storage file may need manual cleanup.";
    clearApprovedWallpapersCache();
    sendJson(res, 200, payload);
  } catch (error) {
    sendJson(res, 400, {
      error: friendlySupabaseError(error) || "Could not delete wallpaper.",
    });
  }
}
async function deleteOwnLocalWallpaper(res, id, user) {
  try {
    const removed = await mutateLocalDb((db) => {
      const index = db.wallpapers.findIndex(
        (entry) => entry.id === id && entry.creatorId === user.creatorId,
      );
      if (index === -1) {
        const error = new Error("Wallpaper not found.");
      ...
      const [item] = db.wallpapers.splice(index, 1);
      const stillReferenced = db.wallpapers.some(
        (entry) => entry.filename === item.filename,
      );
      return { item, stillReferenced };
    });
    ...
    sendJson(res, error.status || 400, {
      error: error.message || "Could not delete wallpaper.",
    });
  }
}
async function deleteSupabaseWallpaper(res, id) {
  try {
    ...
    sendJson(res, 200, {
      ok: true,
      status: "rejected",
      message: "Wallpaper deleted from the app and moved to Rejected.",
    });
  } catch (error) {
    sendJson(res, 400, {
      error: friendlySupabaseError(error) || "Delete failed.",
    });
  }
}
async function deleteLocalWallpaper(res, id) {
  try {
    const removed = await mutateLocalDb((db) => {
      ...
      item.updatedAt = new Date().toISOString();
      const stillReferenced = db.wallpapers.some(
        (entry) => entry.id !== id && entry.filename === previousFilename,
      );
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      !passesOriginCsrfCheck(req)
    ) {
      return sendJson(res, 403, {
        error: "Security check failed. Refresh the page and try again.",
      });
    }
    if (req.method === "POST" && pathname === "/api/check-username")
      return checkUsername(req, res);
    if (req.method === "POST" && pathname === "/api/create-user")
    ...
    if (req.method === "POST" && pathname === "/api/profile/avatar")
      return updateProfilePicture(req, res);
    ...
    if (req.method === "GET" && publicUserMatch)
      return getPublicUserProfile(req, res, publicUserMatch[1]);
    const profileWallpaperUpdateMatch = pathname.match(
      /^\/api\/profile\/wallpapers\/([a-f0-9-]+)\/update$/,
    );
    if (req.method === "POST" && profileWallpaperUpdateMatch)
      return updateOwnWallpaperDetails(
        req,
        res,
        profileWallpaperUpdateMatch[1],
      );
    const profileWallpaperDeleteMatch = pathname.match(
      /^\/api\/profile\/wallpapers\/([a-f0-9-]+)\/delete$/,
    );
    if (req.method === "POST" && profileWallpaperDeleteMatch)
      return deleteOwnWallpaper(req, res, profileWallpaperDeleteMatch[1]);
    if (req.method === "POST" && pathname === "/api/upload")
      return handleUpload(req, res);
    if (req.method === "GET" && pathname === "/api/wallpapers")
      return listApproved(req, res);
    if (req.method === "POST" && pathname === "/api/admin/login")
      return login(req, res);
    if (req.method === "POST" && pathname === "/api/admin/logout")
      return logout(req, res);
    if (req.method === "GET" && pathname === "/api/admin/pending")
      return listPending(req, res);
    if (req.method === "GET" && pathname === "/api/admin/storage")
      return storageStatus(req, res);
    if (req.method === "GET" && pathname === "/api/admin/users")
      return listAdminUsers(req, res);
    if (
      req.method === "POST" &&
      pathname === "/api/admin/users/cleanup-stale-locks"
    )
      return cleanupAdminUserLocks(req, res);
    const adminUserUploadLimitMatch = pathname.match(
      /^\/api\/admin\/users\/([^/]+)\/upload-limit$/,
    );
    if (req.method === "POST" && adminUserUploadLimitMatch)
      return updateAdminUserUploadLimit(req, res, adminUserUploadLimitMatch[1]);
    const adminUserDeleteMatch = pathname.match(
      /^\/api\/admin\/users\/([^/]+)\/delete$/,
    );
    if (req.method === "POST" && adminUserDeleteMatch)
      return deleteAdminUser(req, res, adminUserDeleteMatch[1]);
    const updateMatch = pathname.match(
      /^\/api\/admin\/wallpapers\/([a-f0-9-]+)\/update$/,
    );
    if (req.method === "POST" && updateMatch)
      return updateWallpaperDetails(req, res, updateMatch[1]);
    const moderationMatch = pathname.match(
      /^\/api\/admin\/wallpapers\/([a-f0-9-]+)\/(approve|reject)$/,
    );
    if (req.method === "POST" && moderationMatch) {
      return moderate(
        ...
        res,
        moderationMatch[1],
        moderationMatch[2] === "approve" ? "approved" : "rejected",
      );
    }
    const deleteMatch = pathname.match(
      /^\/api\/admin\/wallpapers\/([a-f0-9-]+)\/delete$/,
    );
    if (req.method === "POST" && deleteMatch)
      return deleteWallpaper(req, res, deleteMatch[1]);
    const downloadMatch = pathname.match(/^\/api\/download\/([a-f0-9-]+)$/);
    if (req.method === "GET" && downloadMatch)
      return downloadWallpaper(req, res, downloadMatch[1]);
    const mediaMatch = pathname.match(/^\/media\/([a-f0-9-]+)$/);
    if (req.method === "GET" && mediaMatch)
      return serveMedia(req, res, mediaMatch[1]);
    const profilePicMatch = pathname.match(/^\/profile-pics\/([^/]+)$/);
    if (req.method === "GET" && profilePicMatch)
      return serveLocalProfilePic(req, res, profilePicMatch[1]);
...
server.listen(PORT, HOST, () => {
  ...
  console.log(
    `Storage mode: ${USE_SUPABASE ? "Supabase Storage + Supabase DB" : "local development fallback"}`,
  );
  if (USE_SUPABASE) {
