...
async function loadQueue() {
  ...
  grid.innerHTML = data.pending.map((item) => `
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        if (button.dataset.action === 'save') {
          ...
          setTimeout(() => { button.textContent = 'Save details'; button.disabled = false; }, 900);
          return;
        }
        if (button.dataset.action === 'approve') {
          await saveWallpaperDetails(button.dataset.id);
        }
        await api(`/api/admin/wallpapers/${button.dataset.id}/${button.dataset.action}`, { method: 'POST' });
        await refreshAll();
      } catch (error) {
...
async function loadExisting() {
  ...
  grid.querySelectorAll('button[data-action="save-existing"]').forEach((button) => {
    button.addEventListener('click', async () => {
      button.disabled = true;
      try {
        await saveWallpaperDetails(button.dataset.id);
  ...
  grid.querySelectorAll('button[data-action="delete-existing"]').forEach((button) => {
    button.addEventListener('click', async () => {
      const ok = confirm('Delete this wallpaper from the app? It will be counted as Rejected on the creator profile.');
      if (!ok) return;
      button.disabled = true;
      try {
        await api(`/api/admin/wallpapers/${button.dataset.id}/delete`, { method: 'POST' });
        await refreshAll();
      } catch (error) {
        showResponseToast(error.message);
        button.disabled = false;
...
async function loadUsers() {
  ...
  const data = await api(`/api/admin/users${query ? `?q=${encodeURIComponent(query)}` : ''}`);
  ...
  grid.querySelectorAll('button[data-action="delete-user"]').forEach((button) => {
    button.addEventListener('click', async () => {
      const username = button.dataset.username || 'this user';
      const ok = confirm(`Delete @${username} completely? This removes the profile, all their wallpapers, profile picture, active login lock, and this profile's IP/browser signup lock. This cannot be undone.`);
      if (!ok) return;
      button.disabled = true;
      try {
        const result = await api(`/api/admin/users/${encodeURIComponent(button.dataset.id)}/delete`, { method: 'POST' });
...
async function cleanupStaleLocks() {
  ...
  const status = $('#userStatus');
  const ok = confirm('Clean only stale/orphaned locks? Active users will keep their IP/device locks.');
  if (!ok) return;
  if (button) button.disabled = true;
  if (status) status.textContent = 'Cleaning stale locks...';
  try {
    const result = await api('/api/admin/users/cleanup-stale-locks', { method: 'POST' });
    if (status) status.textContent = `${result.message || 'Cleaned stale locks.'} Malformed signup rows cleared: ${result.clearedMalformedSignupRows || 0}. Orphaned active sessions cleared: ${result.clearedOrphanedActiveSessions || 0}.`;
    await loadUsers();
  } catch (error) {
...
async function saveWallpaperDetails(id) {
  const titleInput = document.querySelector(`[data-edit="title"][data-id="${id}"]`);
  const creatorInput = document.querySelector(`[data-edit="creator"][data-id="${id}"]`);
